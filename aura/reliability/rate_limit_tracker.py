...
formatted display for /usage-style commands and status-bar summaries.

Supports the Nous Portal header format (also used by OpenRouter and
OpenAI-compatible APIs that follow the same convention):
...
Ported from Hermes Agent (MIT License), Nous Research.
Source: https://github.com/NousResearch/Hermes-Agent/blob/main/agent/rate_limit_tracker.py
"""
...
def format_rate_limit_compact(state: RateLimitState) -> str:
    ...
    rm, tm = state.requests_min, state.tokens_min
    rh, th = state.requests_hour, state.tokens_hour
