...
| `recentTopics` keyword quality | STOP_WORDS expanded 2026-07-29 but the heuristic is fundamentally a word-frequency counter over titles + summaries. Expect low-signal keywords to still appear. |
...
- 12 untested ViewModels (of ~30 total), ~26 untested screens/ routes. ViewModels with
  business logic are tested; Compose UI screens are harder to unit-test meaningfully and
  are exercised through manual use. Carried across multiple passes without movement on
  the screen-testing front — the ROI of Compose UI unit tests on a personal app is low.
- ~106 unlogged `runCatching` sites remain. Most are JSON-parse fallbacks where a sentinel
  return is correct; the network-fallback cases are the ones worth logging. Reduced from
  ~147 by prior passes.
