...
| Item | Detail |
|------|--------|
| Room schema exports 7–10 missing | `aura-core/schemas/com.aura.memory.MemoryDatabase/` jumps 6 → 11. Migration tests can't verify those steps; schema drift is undetectable across that range. |
| 8 of 48 entities have no backup class | Data is permanently lost on backup/restore. Identified in the 07-26 data-layer audit; needs an entity-by-entity roundtrip pass. |
| `runBlocking` inside `runInterruptible` | `ToolExecutor` cancels tools via thread interruption, which only unblocks *blocking* calls. A properly-suspending tool ignores it, so the timeout is weaker than its comment claims. Also pins an IO thread per tool for its full duration — parallel tool execution can starve the pool. |
