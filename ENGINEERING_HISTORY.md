...
Verified against HEAD at the time of writing. This is the section to maintain.

### Correctness

| Item | Detail |
|------|--------|
| Room schema exports 7–10 missing | `aura-core/schemas/com.aura.memory.MemoryDatabase/` jumps 6 → 11. Those files were never committed and are not in git, so they cannot be recovered — only regenerated by checking out the revision where the DB was at each version and building. Consequence: no test can call `createDatabase(name, 7..10)`. Mitigated, not fixed: a v6→v14 full-chain test now forces every migration in 6..14 to run and validates the end state against 14.json. What is still missing is a per-step data assertion for an individual hop inside 6..10. |
| `ToolExecutor` pins an IO thread per tool | `runInterruptible(Dispatchers.IO) { runBlocking { … } }` occupies an IO thread for the tool's whole duration, including for purely-suspending tools that would otherwise release it. With parallel tool execution this can starve the 64-thread IO pool. **Cancellation itself is correct** — see `ToolExecutorCancellationProbeTest`. |
| AgentRun approval loop is one-way | `requestApproval()` creates records and `approve()`/`deny()` update them, but nothing retries the failed step afterwards. Permission-requiring tools always fail inside pipelines. |
| Step counter consumed on failover | The loop increments `step` before the failover inner loop, so a 2-model failover burns 2 of 10 steps for no output. |
| `recentTopics` keyword quality | STOP_WORDS expanded 2026-07-29 but the heuristic is fundamentally a word-frequency counter over titles + summaries. Expect low-signal keywords ("using", "would") to still appear. A future pass could swap to a keyword-extraction model or integrate the taste engine's learned preference signals. |
| 10 separate Room databases | No cross-database transactions or joins, 10 independent migration chains. `BackupManager.kt` is the largest file at ~1,257 lines. Consolidation is a large but bounded change. |
| Dependencies roughly 2 years stale | Kotlin 1.9.24, AGP 8.2.2, Compose BOM 2024.10.01 — pre-K2 compiler. |
| `pendingPermission` is a single global slot | On a `@Singleton`, one permission request at a time. Second conversation hitting a permission gate silently overwrites the first's snapshot. |

### Coverage

- 10 untested ViewModels, 45 untested screens. Carried across four passes without
  movement.
- ~106 unlogged `runCatching` sites remain. Most are JSON-parse fallbacks where a sentinel
  return is correct; the network-fallback cases are the ones worth logging.
- Known-flaky tests dependent on real time: `ProactiveBootstrapTest` polling,
  `MoaProviderTest` dispatch timeout, `ToolExecutorTimeoutTest` `Thread.sleep`,
  `ModelCatalogRepositoryTest`. Each needs a `TestDispatcher` refactor.
- Several test files share mutable mocks across cases — a real isolation risk.

### Architecture

- **10 separate Room databases.** No cross-database transactions or joins, 10 independent
  migration chains, global search fans out across all of them, and backup must coordinate
  10 schemas — which is why `BackupManager.kt` is the largest file in the project at
  ~1,200 lines. Consolidating to one or two databases is a large but bounded change.
- **Dependencies roughly two years stale.** Kotlin 1.9.24, AGP 8.2.2, Compose BOM
  2024.10.01 — pre-K2 compiler.
- **`pendingPermission` is a single global slot** on a `@Singleton`, so exactly one
  permission request can be in flight app-wide.
- **Scope versus depth.** 61 tools, 17 providers, 22 screens, plus evolution, dream, taste,
  world model, creative council, production pipelines, agent DAG runs, and an MCP client —
  for a single-user personal app. Several of these subsystems are 200–500 lines: the
  surface area is large relative to the depth behind each. This was the 07-17 assessment's
  central diagnosis ("too much honest substrate, not enough honest surface") and it has
  not changed in ten passes.
