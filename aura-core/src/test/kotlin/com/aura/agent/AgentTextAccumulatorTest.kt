...
class AgentTextAccumulatorTest {
    ...
    @Test fun `GateRequested does not change text`() {
        ...
        // by the typed gate pause — `GateRequested` — which the loop emits
        // when it pauses for a permission/confirmation/approval grant. The
        // accumulator still ignores it — text is generated by `TextDelta`,
        // not by control events.
        assertEquals(
            "abc",
            AgentTextAccumulator.apply(
                "abc",
                AgentEvent.GateRequested(
                    toolName = "echo",
                    toolCallId = "tc1",
                    args = "{}",
                    kind = MemoryAugmentedAgenticLoop.GateKind.PERMISSION,
                    permission = "perm",
                    rationale = "rationale",
                ),
            ),
