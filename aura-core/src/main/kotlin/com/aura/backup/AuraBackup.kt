...
data class AuraBackup(
    ...
    /**
     * Tables whose rows could not be read when this backup was written.
     *
     * Export is deliberately non-strict: a transient read failure costs those rows rather
     * than the whole backup. What made that dangerous was that it left no trace — a backup
     * missing six tables serialised identically to a complete one, and the worker still
     * recorded "wrote N KB". The loss then surfaced at restore, too late to retake it.
     *
     * Empty for a complete backup, and for every backup written before schema 30.
     */
    val unreadableTables: List<kotlin.String> = emptyList(),
    val exportedAt: Long,
...
data class ForumPostBackup(
    /**
     * The post's row id, which its votes reference.
     *
     * Absent until schema 30, and its absence is why forum votes never survived a restore:
     * posts came back with fresh autogenerated ids while votes still carried the exporting
     * device's, so every `forum_votes.postId` pointed at nothing. Defaulted so a backup
     * written before this deserialises unchanged — those votes are unrecoverable and are
...
fun com.aura.agent.forum.ForumPostEntity.toBackup() = ForumPostBackup(
    id = id, threadId = threadId, agentId = agentId, replyToId = replyToId,
