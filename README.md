...
- **Record mode** (Hands → Record a hand): demonstrate a task once in another app and Aura writes the Hand. Capture is snapshot-diff — the accessibility service forwards `TYPE_WINDOW_CONTENT_CHANGED` as a bare "look now" tick carrying the package name and nothing else, and only while a recording is running, so no new event types are subscribed and no keystroke is ever delivered. `StepInference` reads typing, scrolling and taps from before/after `UiSnapshot` pairs, and **declines rather than guesses**: tapping "Send" usually leaves "Send" on screen, so most taps arrive listing every clickable that could have been pressed, and the review screen asks. A step whose target was never chosen refuses to compile. Recorded steps carry a `selector` rather than an element index — `screen_act` gained that path, since an index names a position in a snapshot that stopped existing — and resolve against a fresh read at replay, failing closed when nothing scores above a text-or-id match or when two candidates tie. Nothing is recorded while a password field is visible, recording never binds to Aura or anything else on the screen-control denylist, and it stops itself at 50 steps. **Recorded hands cannot be scheduled**: `screen_act` needs a session a person opens, so `HandScheduler` returns no run time for any hand that drives the screen — the alternative is a run that does nothing at 09:00 every day forever
...
- Agent runs (durable, resumable, DAG-resolved step execution via WorkManager, approval flow, in-app run timeline). **Resume is re-derivation, not a snapshot**: `DagResolver.readySteps` recomputes which steps are runnable from persisted step status, so there is nothing to go stale. The `checkpoint()` that wrote a snapshot had no reader and is gone — see ENGINEERING_HISTORY §3. A run that cannot proceed says which of the three reasons it is: a dependency that failed, one that is not in the run, or a cycle
...
- 3,641 unit tests, 0 failures (checked against the JUnit XML by `scripts/check-test-count.sh` in CI)
...
- **Agent Runs** — durable run history, with a per-run timeline and resume.
...
|---|---|---|
| MemoryDatabase | v32 | Memories, memory edits, document chunks, creative artifacts/revisions/branches/jobs, canon facts/simulations/continuity, beliefs/evidence/events/opportunities, preference signals/style profiles/reference identities/routing outcomes, FTS4 index over memory content, coarse place visits, creative analysis per revision |
...
schema exports span versions 1-32, all committed — migration tests cover
the full chain.
...
- Room 2.8.4 (11 databases, 69 entities, 56 migrations, schema export)
