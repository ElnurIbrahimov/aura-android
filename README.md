...
- 3,157 unit tests, 0 failures (checked against the JUnit XML by `scripts/check-test-count.sh` in CI)
- 72 instrumented test methods (29 Room migration-chain methods in :aura-core, 43 in :app — 38 Compose rendering, 5 device smoke). The :aura-core migration suites now **execute** in CI on an emulator (the `migrations` job); :app's are still **compiled** only, and run via `connectedAndroidTest` on a device
...
schema exports span versions 1-23, all committed — migration tests cover
the full chain.
...
- Room 2.8.4 (11 databases, 63 entities, 47 migrations, schema export)
