...
- 3,437 unit tests, 0 failures (checked against the JUnit XML by `scripts/check-test-count.sh` in CI)
...
|   32 nav destinations + 42 ViewModels      |
+------------+-------------------------------+
             | depends on
+------------v-------------------------------+
