...
- 3,408 unit tests, 0 failures (checked against the JUnit XML by `scripts/check-test-count.sh` in CI)
...
- 82 instrumented test methods (35 Room migration-chain methods in :aura-core, 47 in :app — 37 Compose rendering, 5 launch and flow, 5 device smoke). Both suites **execute** in CI on an emulator: `:aura-core` in the `migrations` job, `:app` in `app-instrumented`, which excludes `DeviceSmokeTest` by name because it needs a real key. The breakdown above said 31 and 43 while the tree held 33 and 45 — only the total is gated, so the halves drifted unwatched, which is the rot the gate exists to catch happening in a number it does not look at
