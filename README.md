...
- 3,630 unit tests, 0 failures (checked against the JUnit XML by `scripts/check-test-count.sh` in CI)
...
|   33 nav destinations + 43 ViewModels      |
+------------+-------------------------------+
             | depends on
+------------v-------------------------------+
