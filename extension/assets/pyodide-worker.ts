...
async function installPackages(packages: string[]): Promise<void> {
  const toInstall = packages.filter(p => !STDLIB.has(p) && MICROPIP_PACKAGES.has(p));
  ...
  for (const pkg of toInstall) {
    try {
      ...
      try {
        pyodide.pyimport(pkg === 'sklearn' ? 'sklearn' : pkg);
...
`;
    ...
    // Extract variable state (user-defined only)
    const varsCode = `
import json as _json
_aura_vars = []
_skip = {'_sys', '_io', '_plt', '_orig_show', '_aura_show', '_aura_images', '_buf', '_json', '_aura_vars', '_skip'}
